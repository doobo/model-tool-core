package javax.doobo.map;


import javax.doobo.model.DoAssert;
import javax.doobo.utils.DoObjectUtils;
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    public ConcurrentReferenceHashMap(int initialCapacity, float loadFactor, int concurrencyLevel, ConcurrentReferenceHashMap.ReferenceType referenceType) {
        DoAssert.isTrue(initialCapacity >= 0, "Initial capacity must not be negative");
        DoAssert.isTrue(loadFactor > 0.0F, "Load factor must be positive");
        DoAssert.isTrue(concurrencyLevel > 0, "Concurrency level must be positive");
        DoAssert.notNull(referenceType, "Reference type must not be null");
    ...
    public boolean containsKey(Object key) {
        ConcurrentReferenceHashMap.Entry<K, V> entry = this.getEntryIfAvailable(key);
        return entry != null && DoObjectUtils.nullSafeEquals(entry.getKey(), key);
    }
    ...
    public boolean remove(Object key, final Object value) {
        return (Boolean)this.doTask(key, new ConcurrentReferenceHashMap<K, V>.Task<Boolean>(new ConcurrentReferenceHashMap.TaskOption[]{ConcurrentReferenceHashMap.TaskOption.RESTRUCTURE_AFTER, ConcurrentReferenceHashMap.TaskOption.SKIP_IF_EMPTY}) {
            protected Boolean execute(ConcurrentReferenceHashMap.Reference<K, V> ref, ConcurrentReferenceHashMap.Entry<K, V> entry) {
                if (entry != null && DoObjectUtils.nullSafeEquals(entry.getValue(), value)) {
                    ref.release();
                    return true;
                } else {
                    return false;
    ...
    public boolean replace(K key, final V oldValue, final V newValue) {
        return (Boolean)this.doTask(key, new ConcurrentReferenceHashMap<K, V>.Task<Boolean>(new ConcurrentReferenceHashMap.TaskOption[]{ConcurrentReferenceHashMap.TaskOption.RESTRUCTURE_BEFORE, ConcurrentReferenceHashMap.TaskOption.SKIP_IF_EMPTY}) {
            protected Boolean execute(ConcurrentReferenceHashMap.Reference<K, V> ref, ConcurrentReferenceHashMap.Entry<K, V> entry) {
                if (entry != null && DoObjectUtils.nullSafeEquals(entry.getValue(), oldValue)) {
    ...
    private class EntryIterator implements Iterator<java.util.Map.Entry<K, V>> {
        ...
        public void remove() {
            DoAssert.state(this.last != null, "No element to remove");
    ...
    private class EntrySet extends AbstractSet<java.util.Map.Entry<K, V>> {
        ...
        public boolean contains(Object o) {
            if (o instanceof java.util.Map.Entry) {
                ...
                if (otherEntry != null) {
                    return DoObjectUtils.nullSafeEquals(otherEntry.getValue(), otherEntry.getValue());
                }
            }
    ...
    protected static final class Entry<K, V> implements java.util.Map.Entry<K, V> {
        ...
        public final boolean equals(Object other) {
            if (this == other) {
                ...
                return DoObjectUtils.nullSafeEquals(this.getKey(), otherEntry.getKey()) && DoObjectUtils.nullSafeEquals(this.getValue(), otherEntry.getValue());
            }
        }

        public final int hashCode() {
            return DoObjectUtils.nullSafeHashCode(this.key) ^ DoObjectUtils.nullSafeHashCode(this.value);
        }
    }
            ...
            for(ConcurrentReferenceHashMap.Reference currRef = ref; currRef != null; currRef = currRef.getNext()) {
                if (currRef.getHash() == hash) {
                    ...
                    if (entry != null) {
                        ...
                        if (DoObjectUtils.nullSafeEquals(entryKey, key)) {
                            return currRef;
                        }
                    }
                }
