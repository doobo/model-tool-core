...
public class WeightRandom<T extends Serializable> {
    ...
    protected List<WeightElement<T>> origin;

    protected int sum = 0;

    public WeightRandom(List<WeightElement<T>> list){
        DoAssert.notEmpty(list,"数据源不能为空");
        origin = list;
        setMinAndMaxAndSum();
    }
    ...
    public List<T> getNoRepeatElementsByFixedUnsafe(int amount){
        DoAssert.isTrue(WeightRandomUtils.hasValue(origin),"无数据源");
        long count = origin.stream().filter(Objects::nonNull).filter(f->f.getCount()>0).count();
        DoAssert.isTrue(count>=amount,"数据源不够！");
    ...
    public T getOneUnsafe(){
        long count = origin.stream().filter(Objects::nonNull).filter(f->f.getCount()>0).count();
        DoAssert.isTrue(count > 0, "数据源不够");
        WeightElement<T> weightElement = getRandomElementByBinary();
        DoAssert.notNull(weightElement,"数据拷贝失败，请检测克隆方法");
    ...
    private WeightElement<T> binarySearch(List<WeightElement<T>> list, int key){
        DoAssert.isTrue(WeightRandomUtils.hasValue(list),"参数异常");
        int lo = 0;
        int hi = list.size() - 1;
        while(lo<=hi){
            int mid = lo + (hi-lo)/2;
